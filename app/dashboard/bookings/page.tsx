...
import { Loader2, CalendarDays, Clock, User, Phone, Mail, XCircle, Trash2, Search, FilterX, Briefcase } from "lucide-react";
...
export default function IncomingBookingsPage() {
    ...
    const fetchIncomingBookings = async () => {
        try {
            ...
            toast.error("Error loading incoming bookings");
        } finally {
            setLoading(false);
        }
    };

    useEffect(() => { fetchIncomingBookings(); }, []);
    ...
    const handleDelete = async (id: any) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                fetchIncomingBookings();; // إعادة تحميل القائمة
            }
        } catch (error) {
                                            ...
                                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-3">
                                                ...
                                                <div className="space-y-2">
                                                    <div className="flex items-center gap-2 text-sm font-bold text-zinc-800">
                                                        <User className="h-4 w-4 text-zinc-400" /> {booking.clientId?.name}
                                                    </div>
                                                    <div className="flex items-center gap-2 text-sm text-zinc-600">
                                                        <Mail className="h-4 w-4 text-zinc-400" /> {booking.clientId?.email}
                                                    </div>
                                                    {clientPhone && (
                                                        <div className="flex items-center gap-2 text-sm font-bold text-emerald-600">
                                                            <Phone className="h-4 w-4" /> {clientPhone}
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
