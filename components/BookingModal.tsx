...
export default function BookingModal({
    ...
    const generateTimeSlots = (from: string | undefined, to: string | undefined) => {
        ...
        const slots = [];
        // نستخدم Math.max و Math.min لضمان عدم حدوث Loop لانهائي
    ...
    // توليد المواعيد بناءً على الخدمة المختارة حالياً
    ...
    const handleConfirmBooking = async () => {
        ...
        // 1. دمج التاريخ المختار مع الساعة المختارة
        const [hours, minutes] = time.split(":").map(Number);
        const startDateTime = new Date(date);
        startDateTime.setHours(hours, minutes, 0, 0);

        // 2. فحص الأمان: هل الوقت المختار أصبح في الماضي؟
        const now = new Date();
        if (startDateTime < now) {
            toast.error("This time has already passed. Please select a future time.");
            // اختياري: مسح الوقت المختار لإجبار المستخدم على اختيار وقت جديد
            setTime(""); 
            return;
        }
        ...
        try {
            const res = await fetch("/api/bookings", {
                ...
                body: JSON.stringify({
                    ...
                    // نرسل أيضاً الـ providerId لأنه مطلوب في الـ Schema كما رأينا سابقاً
                    providerId: service.ownerId?._id || service.ownerId, 
                    endTime: new Date(startDateTime.getTime() + 60 * 60 * 1000).toISOString(), // افتراض أن الخدمة ساعة واحدة
                }),
            });
            ...
            if (res.ok) {
                ...
                toast.error(errorData.message || "Booking failed. Try another slot.");
            }
        } catch (error) {
                ...
                <div className="p-6 space-y-6">
                    ...
                    <div className="space-y-3">
                        ...
                        <Select onValueChange={setTime} value={time}>
                            ...
                            {/* هنا تم حل مشكلة الظهور خلف المودال */}
